...
import cn.ufunny.game.libs.cache.support.annotation.AbstractAlterableValueCacheSupport;
 ...
 * @param <V>
 */
public abstract class EhAlterableValueCacheSupport<K, V> extends AbstractAlterableValueCacheSupport<K, V>
	...
	protected <T> boolean removeNameKey(String name,T t){
		String key = buildKey(name, t);
	...
	public boolean put(V value) {
		K k = findDefaultKey(value);
		boolean success = wrapper.putByKey(k,value);
		List<String> keys = findCacheKeys(value);
		for(String key : keys){
