 ...
 * @param <V>
 */
public abstract class EhAlterableValuesCacheSupport<K, T, V extends ValueIdentifiable<K,T>> extends AbstractAlterableValuesCacheSupport<K,T, V>
	...
	public boolean put(V value) {
		...
		K key = value.findIdentify();
		T valueKey = value.findValueIdentify();
		boolean success = wrapper.putEhcacheValue(key,valueKey,value);
	...
	public boolean putIfAbsent(V value) {
		...
		K key = value.findIdentify();
		T valueKey = value.findValueIdentify();
		boolean success = wrapper.putIfAbsentEhcacheValue(key,valueKey,value);
		List<String> cacheKeys = value.findFieldIdentifies();
		if(cacheKeys != null){
			for(String cacheKey : cacheKeys){
	...
	public boolean remove(V value) {
		...
		K key = value.findIdentify();
		boolean success = wrapper.removeByKey(key);
		List<String> cacheKeys = value.findFieldIdentifies();
		if(cacheKeys != null){
			for(String cacheKey : cacheKeys){
