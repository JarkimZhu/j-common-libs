...
import cn.ufunny.game.libs.cache.inherit.MultipleIdentifiable;
 ...
 * @param <V>
 */
public abstract class EhAlterableMultipleKeyValueCacheSupport<K, V extends MultipleIdentifiable<K>> extends AbstractAlterableValueCacheSupport<K, V>
	...
	public boolean put(V value) {
		...
		for(K k : ks){
			success = wrapper.putByKey(k,value) && success;
		}
	...
	public boolean putIfAbsent(V value) {
		...
		for(K k : ks){
			wrapper.putIfAbsentByKey(k,value);
		}
		List<String> keys = value.findFieldIdentifies();
		if(keys != null){
			for(String key : keys) {
	...
	public boolean remove(V value) {
		...
		for(K k : ks){
			success = wrapper.removeByKey(k) && success;
		}
		List<String> keys = value.findFieldIdentifies();
		if(keys != null){
			for(String key : keys){
				success = wrapper.removeByKey(key) && success;
			}
		}
		return success;
	}
